...
public class PolyGenJSON {
  public static void main(String[] args) {
    ...
    try {
      ...
      writer.write("{\n");
      writer.write("   \"type\": \"MultiPolygon\",\n");
      ...
      for(Polygon poly : polys) {
        ...
        for(Coordinate coord : poly.getExteriorRing().getCoordinates()) {
          String comma = (c < poly.getExteriorRing().getCoordinates().length) ? "," : "";
          lineStr += "[ "+String.format("%.6f", coord.x)+", "+String.format("%.6f", coord.y)+" ]"+comma+" ";
        ...
        lineStr += "]" + (poly.getNumInteriorRing() > 0 ? "," : "");
        ...
        if(poly.getNumInteriorRing() > 0) {
          for(int iri=0; iri < poly.getNumInteriorRing(); iri++) {
            ...
            for(Coordinate coord : poly.getInteriorRingN(iri).getCoordinates()) {
              String comma = (c < poly.getInteriorRingN(iri).getCoordinates().length) ? "," : "";
              lineStr += "[ "+String.format("%.6f", coord.x)+", "+String.format("%.6f", coord.y)+" ]"+comma+" ";
              c++;
            }
            lineStr += "]" + (iri+1 < poly.getNumInteriorRing() ? "," : "");
            writer.write(lineStr+"\n");
          }
        }
        writer.write("        ]"+(polyCount < polys.size() ? "," : "")+"\n");
        polyCount++;
      }
